...
const DonutChart = React.createClass({
  ...
  render() {
    ...
    return (
      <svg width={this.props.size} height={this.props.size} className="rh-donutchart">
        ...
        <text className="rh-donutchart-text" x={halfsize} y={halfsize+8} style={{textAnchor:'middle'}} >
          <tspan className="rh-donutchart-text-val">{this.props.value}</tspan>
          <tspan className="rh-donutchart-text-percent">%</tspan>
          <tspan className="rh-donutchart-text-label" x={halfsize} y={halfsize+10}>{this.props.valuelabel}</tspan>
        </text>
      </svg>
    );
  }
